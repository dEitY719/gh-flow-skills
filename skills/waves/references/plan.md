...
One `gh issue view <N> --repo "$TARGET_REPO" --json body,comments` per
...
- **Explicit dependencies** — `Depends on #M`, `Blocked by #M`, `#M 이 먼저`,
  ...
  GitHub's `blockedBy` links (`gh api graphql` on `issue.blockedBy`). Comments
...
- **Epic** — an issue whose body carries a child table / task list of `#N`
  ...
  child is closed (`gh issue close <N> --repo ... --comment "<children and their PRs>"`).
...
4. File overlap: two issues in the same wave that name the same file are split —
   ...
   brief as **files overlapping sibling workers**.
...
`gh pr list --repo "$TARGET_REPO" --state open --json number,headRefName,closingIssuesReferences`
...
Per issue in the wave, from the main checkout:
`Skill(session:worktree-spawn, "--task issue-<N>")`. Record the `Path:` and
