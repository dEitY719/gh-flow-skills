...
| - | `--trust-origin` | off | Skip Step 1.5's origin trust gate entirely — no fetch, no parse, no validity review. The escape hatch for a backlog of unknown-origin issues (dEitY719/gh-flow-skills#36 F-8) |
...
- `/gh-flow:issue 16 --trust-origin` — same chain, with Step 1.5 skipped: implement starts even if the issue has no origin line or was written by an untrusted harness.
...
|---|---|---|
| `GH_TRUSTED_HARNESSES` | `claude codex` | Space-separated allow-list of harnesses whose issues skip Step 1.5's validity review. Family-wide — `gh-issue:*` reads the same variable (dEitY719/gh-issue-skills#44 D-3), so the trust set is configured once per machine, not once per repo. Set it to the empty string to review every issue; an unknown origin (`none`, no line, failed fetch) is never trusted whatever it contains |

## Step 1.5: the origin trust gate

Before any file is edited, the chain reads the issue body's
`Generated by Harness(<harness>) and LLM(<model>)` line (format owned by
dEitY719/gh-issue-skills#43) and routes on who wrote the spec it is about to
implement:

- **Trusted harness** (`GH_TRUSTED_HARNESSES`, default `claude codex`) — straight
  to step 1 below, no review.
- **Anything else** — an untrusted harness, `Harness(none)`, a malformed line, an
  issue predating the origin line, or a failed fetch — gets a six-check validity
  review first (contradictions, phantom paths, unverifiable acceptance criteria,
  already-done/duplicate, safety-contract violations, oversized scope). PASS
  starts the chain; BLOCK stops it with **zero edits, zero commits, zero PRs**,
  exit 2, and a comment on the issue saying which checks failed.
- **`--trust-origin`** — gate skipped entirely.

Full procedure and tables: `references/origin-trust-gate.md`.
